...
class ConfiguratorExtractor:
    ...
    def extract_and_normalize(
        ...
        if raw_price_file.exists():
            with open(raw_price_file, "r", encoding="utf-8") as f:
                ...
                for row in reader:
                    ...
                    src_url = (
                        row.get("source_url")
                        or f"https://shop.vinfastauto.com/vn_vi/dat-coc-o-to-dien-vinfast.html?modelId=Products-Car-{m_id}"
                    )
        ...
        if raw_colors_file.exists():
            with open(raw_colors_file, "r", encoding="utf-8") as f:
                ...
                for row in reader:
                    ...
                    src_url = (
                        f"https://shop.vinfastauto.com/vn_vi/dat-coc-o-to-dien-vinfast.html?modelId=Products-Car-{m_id}"
                    )
        ...
        self._write_csv(
            self.out_dir / "edition.csv",
            ["model_id", "edition_id", "model_label", "edition_label", "year_range", "is_active"],
            editions_out,
        )
        self._write_csv(
            self.out_dir / "price_list.csv",
            [
                "model_id",
                "edition_id",
                "price_list_vnd",
