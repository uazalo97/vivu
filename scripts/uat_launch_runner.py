    ...
    for case in cases:
        ...
        for index, step in enumerate(case["steps"], 1):
            ...
            if expected_decisions:
                lines.append(
                    "Decision chấp nhận: "
                    + ", ".join(f"`{decision}`" for decision in expected_decisions)
                    + "."
                )
                lines.append("")
...
def _write_csv_pack(cases: list[dict[str, Any]], output: Path) -> None:
    ...
    def step_value(case: dict[str, Any], index: int, key: str) -> str:
        ...
        if key == "assertions":
            return " | ".join(
                _assertion_note(assertion).replace("`", "")
...
def main() -> int:
    ...
    parser.add_argument("--prepare-only", action="store_true", help="Expand the 25 scenarios into 75 cases without calling API")
    parser.add_argument("--export-copy-pack", action="store_true", help="Export a manual Markdown pack for copy/paste UAT")
