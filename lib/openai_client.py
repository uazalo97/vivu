#!/usr/bin/env python3
"""
openai_client.py — Helpers dùng chung cho OpenAI API.

Thống nhất về 1 OpenAI key duy nhất (OPENAI_API_KEY + OPENAI_BASE_URL).
Trước đây là openrouter.py, đã đổi tên để thuần OpenAI.

Đọc key từ .env (python-dotenv). Model mặc định:
  - Embed : text-embedding-3-small  (1536 chiều)
  - Chat  : gpt-4o-mini
Có thể ghi đè qua biến môi trường OPENAI_EMBED_MODEL / LLM_MODEL.
"""
...
CHAT_REASONING = ""  # reasoning disabled for OpenAI gpt-* (pure OpenAI, no OpenRouter reasoning param)
MAX_RETRIES = 4
