...
async def search_knowledge_base(query: str, model_id: str = None, collections: list[str] | None = None, skip_rerank: bool = False) -> dict:
    from app.core.cache import search_kb_cached

    mid = _model_id(model_id) if model_id else None
    return await search_kb_cached(query, mid, collections, skip_rerank=skip_rerank)
