...
_TOPIC_KEYWORDS = {
    "giá": [
        ...
        r"\bgiá\b", r"bao\s*nhiêu\s*tiền", r"chi\s*phí", r"ưu\s*đãi",
        r"giá\s*(xe|VF)", r"price",
        r"rẻ\s*nhất", r"đắt\s*nhất", r"rẻ\s*hơn", r"đắt\s*hơn",
        r"giá\s*(dưới|trên|khoảng|từ|đến)", r"giá\s*của",
        r"triệu\s*đồng", r"triệu\b", r"tỷ\b",
        r"trả\s*góp", r"vay\s*mua",
    ],
    "pin_và_sạc": [
    ...
    "phạm_vi_di_chuyển": [
        ...
        r"đi\s*được\s*bao\s*km", r"quãng\s*đường",
    ],
    "an_toàn": [
        ...
        r"isofix", r"tpms", r"đai\s*an\s*toàn", r"chống\s*trộm", r"báo\s*động",
    ],
    "màu_sắc": [
        r"màu", r"màu\s*(gì|nào|sắc|xe)", r"bao\s*nhiêu\s*màu", r"mấy\s*màu",
        r"color", r"sơn",
    ],
    "option": [
        r"option", r"tùy\s*chọn", r"tuỳ\s*chọn", r"nâng\s*cấp",
        r"lazang", r"la-zăng", r"mâm\s*(hợp\s*kim|lõi\s*thép)",
        r"trần\s*kính", r"hệ\s*dẫn\s*động",
    ],
    "nội_thất": [
        r"nội\s*thất", r"ghế", r"chỗ\s*ngồi", r"seats", r"seat\s*(count|number)|số\s*chỗ",
        r"mấy\s*chỗ", r"bao\s*nhiêu\s*chỗ", r"\bchỗ\b",
        r"màn\s*hình", r"loa", r"âm\s*thanh",
        r"điều\s*hòa", r"khoang\s*xe", r"vô\s*lăng", r"HUD",
        r"leatherette", r"speaker", r"display", r"sưởi", r"thông\s*gió",
        r"massage", r"cửa\s*sổ\s*trời", r"sunroof",
    ],
    "ngoại_thất": [
        r"ngoại\s*thất", r"đèn", r"màu\s*xe", r"mâm", r"la-zăng",
        r"gương", r"body", r"design", r"kiểu\s*dáng",
        r"headlight", r"tail\s*light", r"DRL", r"lốp", r"tire",
    ],
    "tính_năng_nổi_bật": [
        ...
        r"trợ\s*lý\s*ảo", r"giọng\s*nói", r"voice", r"karaoke",
        r"ứng\s*dụng", r"app", r"web\s*browser", r"kết\s*nối",
        r"điều\s*khiển\s*từ\s*xa", r"esim", r"wifi",
    ],
    "phiên_bản": [
